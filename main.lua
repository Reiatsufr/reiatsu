...
getgenv().ESP = {
    Main = {
        ...
        Name = {
            Enabled = false,-- TOGGLE
            Color = Color3.fromRGB(255, 255, 255), -- COLOR PICKER
        },
        Box = {
            Enabled = false,
            BoxColor = Color3.fromRGB(75, 175, 175), -- COLOR PICKER
            BoxFillColor = Color3.fromRGB(100, 75, 175), -- COLOR PICKER
            FillEnabled = false, -- Added toggle for box fill -- TOGGLE
        },
        HealthBar = {
            Enabled = false,-- TOGGLE
            Number = false,-- TOGGLE 
            HighHealthColor = Color3.new(0.513725, 0.521569, 1.000000), -- COLORPICKER
            LowHealthColor = Color3.new(0.509804, 0.556863, 1.000000), -- COLORPICKER
        },
        Tool = {
            Enabled = false, -- NO TOGGLE
        ...
        Chams = false,-- NO TOGGLE
        AutomaticColor = true, -- THIS WONT HAVE TOGGLE
        Type = "AlwaysOnTop", -- ALWAYS ON TOP ( NO TOGGLE )
    },
    Checks = {
        WallCheck = false,-- TOGGLE
        VisibleCheck = false,-- TOGGLE
        ForceField = false,-- TOGGLE
        AliveCheck = false, -- TOGGLE
    },
    Extra = {
        UseDisplayName = false, -- TOGGLE
        EspFadeOut = 400 -- WILL HAVE SLIDER
    }
}
...
do -- // Utility
    function Utility:Connection(connectionType, connectionCallback)
        ...
        dababy.connections[#dababy.connections + 1] = connection
        --
        return connection
    end
    --
    ...
    function Utility:ThreadFunction(Func, Name, ...)
        local Func = Name and function()
            ...
            --
            if not Passed and not dababy.Safe then
                warn("dababy:\n", "              " .. Name .. ":", Statement)
            end
        end or Func
...
--
do --// Functions
    function dababy:PlayerValid(Player, Function)
        if Player:IsA("Player") then
            if Function then return Function(Player) else return true end
        end
    end
    --
    function dababy:GetCharacter(Player)
        return Player.Character
    end
    --
    function dababy:GetHumanoid(Player, Character)
        return Character:FindFirstChildOfClass("Humanoid")
    end
    --
    function dababy:GetHealth(Player, Character, Humanoid)
        if Humanoid then
            return Clamp(Humanoid.Health, 0, Humanoid.MaxHealth), Humanoid.MaxHealth
        end
    end
    --
    function dababy:GetRootPart(Player, Character, Humanoid)
        return Humanoid.RootPart
    end
    --
    function dababy:GetIgnore(Unpacked)
        return
    end
    --
    function dababy:GetBodyParts(Character, RootPart, Indexes, Hitboxes)
        local Parts = {}
    ...
    --
    function dababy:ClientAlive(Player, Character, Humanoid)
        local Health, MaxHealth = dababy:GetHealth(Player, Character, Humanoid)
        --
        return (Health > 0)
    end
    --
    function dababy:ValidateClient(Player)
        local Object = dababy:GetCharacter(Player)
        local Humanoid = (Object and dababy:GetHumanoid(Player, Object))
        local RootPart = (Humanoid and dababy:GetRootPart(Player, Object, Humanoid))
        --
        return Object, Humanoid, RootPart
    end
    --
    function dababy:GetBoundingBox(BodyParts, RootPart)
        ...
        for Index, Value in pairs({"Head", "Torso", "Left Arm", "Right Arm", "Left Leg", "Right Leg"}) do
            ...
            local PartSize = (Part and Part.Size or dababy.Locals.PartSizes[Value])
            --
            if Value == "Head" then
    ...
    --
    function dababy:RayCast(Part, Origin, Ignore, Distance)
    ...
    --
    function dababy:GetPlayers()
        return Players:GetPlayers()
    end
    --
    function dababy:PlayerAdded(Player)
        Visuals:Create({Player = Player})
    end
    --
    function dababy:GetUserID(Player)
        return Player.UserId
    end
    --
    function dababy:GetPlayerParent(Player)
...
do -- // Visuals
    ...
    function Visuals.Base:Update()
        ...
        if Self then
            ...
            local Parent = dababy:GetPlayerParent(Player)
            --
            if (Player and Player ~= Client and Parent and Parent ~= nil) or (Info.RootPartCFrame and Info.Health and Info.MaxHealth) then
                if ESP.Main.Enabled then
                    local Object, Humanoid, RootPart = dababy:ValidateClient(Player)
                    local BodyParts = (RootPart and dababy:GetBodyParts(Object, RootPart, true))
                    local TransparencyMultplier = 1
                    --
                    if Object and Object.Parent and (Humanoid and RootPart and BodyParts) then
                        local Health, MaxHealth = dababy:GetHealth(Player, Object, Humanoid)
                        --
                        if (ESP.Checks.AliveCheck and not dababy:ClientAlive(Player, Character, Humanoid)) or (ESP.Checks.ForceField and Object:FindFirstChildOfClass("ForceField")) then 
                            Info.Pass = false
                        else
                            Info.Pass = true
                    ...
                    if Info.RootPartCFrame and Info.Health and Info.MaxHealth then
                        ...
                        if OnScreen then
                            ...
                            if Magnitude <= MaxDistance then
                                local BoundingBox = (Info.Pass and {dababy:GetBoundingBox(BodyParts, RootPart)} or Info.BoundingBox)
...
do -- Box
    if ESP.Main.Box.Enabled then
        ...
        if ESP.Main.Box.FillEnabled then -- Check if box fill is enabled
            ...
            SetRenderProperty(Renders.BoxFill, "Visible", false) -- Hide box fill if disabled
        end
    else
        ...
        SetRenderProperty(Renders.BoxFill, "Visible", false)
                        ...
                        do -- Chams
                            if ESP.Main.Chams then
                                ...
                                --
                                local ChamsAuto = dababy.Locals.SelectedPlayersSection ~= "Local" and Flags[Selection .. "ChamsAuto"]:Get()
                                ...
                                --
                                local Visible = OnScreen and (RootPart ~= nil and dababy:RayCast(RootPart, Workspace.CurrentCamera.CFrame.Position, {dababy:GetCharacter(Client), dababy:GetIgnore(true)}))
                                --
                                if Info.Pass then
...
do -- // Connections
    ...
    Utility:Connection(Players.ChildAdded, function(Child)
        dababy:PlayerValid(Child, function(Validated) 
            dababy:PlayerAdded(Validated) 
        end)
    end)
end 
--
for Index, Player in pairs(dababy:GetPlayers()) do
     dababy:PlayerValid(Player, function(Validated) 
        dababy:PlayerAdded(Validated) 
    end)
end
